...
export const exportDrillToGLB = async (
  parameters: DrillParameters,
  filename: string = 'drill.glb',
  ...
  try {
    ...
    // Create a mesh for the GLB export
    ...
    // Rotate the mesh to horizontal position if requested
    if (horizontal) {
    ...
    // Create a scene to hold the mesh and lights
    const scene = new THREE.Scene();
    scene.add(mesh);
    ...
    exporter.parse(
      ...
      (gltfData) => {
        ...
        const blob = new Blob([gltfData as ArrayBuffer], { type: 'application/octet-stream' });
